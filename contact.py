...
def DisplayForm():
    ...
    display_screen.title("DevanshiSojita.com")
    global tree
    global SEARCH
    ...
    #label for heading
    lbl_text = Label(TopViewForm, text="Contact BOOK", font=('verdana', 18), width=600,bg="#25383C", fg="white")
    ...
    Button(LFrom,text="Submit",font=("Arial", 10, "bold"),command=register,bg="#1589FF",fg="white").pack(side=TOP, padx=10,pady=5, fill=X)

    #creating search label and entry in second frame
    lbl_txtsearch = Label(LeftViewForm, text="Enter fname to Search", font=('verdana', 10),bg="#FFFFFF")
    lbl_txtsearch.pack()
    #creating search entry
    search = Entry(LeftViewForm, textvariable=SEARCH, font=('verdana', 15), width=10)
    search.pack(side=TOP, padx=10, fill=X)
    #creating search button
    btn_search = Button(LeftViewForm, text="Search", command=SearchRecord,bg="Skyblue")
    btn_search.pack(side=TOP, padx=10, pady=10, fill=X)
    #creating view button
    btn_view = Button(LeftViewForm, text="View All", command=DisplayData,bg="Skyblue")
    btn_view.pack(side=TOP, padx=10, pady=10, fill=X)
    #creating reset button
    btn_reset = Button(LeftViewForm, text="Reset", command=Reset,bg="Skyblue")
    btn_reset.pack(side=TOP, padx=10, pady=10, fill=X)
    #creating delete button
    btn_delete = Button(LeftViewForm, text="Delete", command=Delete,bg="Skyblue")
    btn_delete.pack(side=TOP, padx=10, pady=10, fill=X)
    #create update button
    btn_delete = Button(LeftViewForm, text="Update", command=Update,bg="Skyblue")
    btn_delete.pack(side=TOP, padx=10, pady=10, fill=X)
    #setting scrollbar
